create database if not exists Furama_managment;
use Furama_managment;
create table vi_tri (
...
select *,year(curdate()) - year(ngay_sinh)  as so_tuoi 
from khach_hang 
where year(curdate()) - year(ngay_sinh)
between 18 and 50 and (dia_chi like ('%Đà Nẵng') or dia_chi  like('%Quãng Trị')); 
...
select kh.ma_khach_hang,kh.ho_ten,lk.ten_loai_khach,hd.ma_hop_dong,dv.ten_dich_vu,hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,ifnull(sum( hdct.so_luong * dvdk.gia)+dv.chi_phi_thue,0) as tổng_tiền
from khach_hang kh
...
select ma_dich_vu from hop_dong where year(ngay_lam_hop_dong) =2021 and month(ngay_lam_hop_dong) between 1 and 3
);

-- 7. Hiển   thị   thông   tin  ma_dich_vu,   ten_dich_vu,   ten_dich_vu,
...
left join loai_khach lk on lk.ma_loai_khach = kh.ma_loai_khach 
where ten_loai_khach = 'diamond'and (dia_chi like '%vinh%' or '%quảng ngãi%');
...
select hd.ma_hop_dong, nv.ho_ten as ' tên nhân viên', kh.ho_ten as 'tên khách hàng', kh.so_dien_thoai,dv.ten_dich_vu,hd.tien_dat_coc,sum(hdct.so_luong) as so_luong_dich_vu_di_kem
from hop_dong hd 
...
select dvdk.ma_dich_vu_di_kem,dvdk.ten_dich_vu_di_kem,sum(hdct.so_luong) as so_lan_dat
from dich_vu_di_kem dvdk
...
select * from v_dich_vu_di_kem where so_lan_dat = ( select max(so_lan_dat) from v_dich_vu_di_kem);
...
select hd.ma_hop_dong,ldv.ten_loai_dich_vu,dvdk.ten_dich_vu_di_kem, dvdk.ma_dich_vu_di_kem as so_lan_su_dung
from hop_dong hd 
...
select  nv.ma_nhan_vien, nv.ho_ten,td.ten_trinh_do,vt.ma_vi_tri,nv.so_dien_thoai,nv.dia_chi,count(hd.ma_hop_dong) as so_luong_hop_dong
from nhan_vien nv
left join hop_dong hd on nv.ma_nhan_vien = hd.ma_nhan_vien
left join trinh_do td on td.ma_trinh_do = nv.ma_trinh_do
left join vi_tri vt  on vt.ma_vi_tri = nv.ma_vi_tri
where year(hd.ma_hop_dong) between 2020 and 2021
group by nv.ma_nhan_vien
having so_luong_hop_dong <=3;

select nv.ma_nhan_vien, nv.ho_ten, td.ten_trinh_do, vt.ten_vi_tri,nv.so_dien_thoai, nv.dia_chi, count(hd.ma_hop_dong) as so_luong_hop_dong
from nhan_vien nv
left join hop_dong hd on nv.ma_nhan_vien = hd.ma_nhan_vien
left join trinh_do  td on td.ma_trinh_do = nv.ma_trinh_do
left join vi_tri vt on vt.ma_vi_tri =nv.ma_vi_tri
...
-- đến năm 2021.
 alter table nhan_vien add column `status` bit default 1;
-- tao stored procedure lay toan bo thong tin nhan vien
DELIMITER $$
create  procedure get_all_info_employee()
begin
